...
remove_managed_npmrc_prefix() {
  ...
  if [[ "$changed" == true ]]; then
    backup="$npmrc.lark-bridge.bak"
    cp "$npmrc" "$backup"
    mv "$tmp_file" "$npmrc"
    log "removed managed npm prefix from $npmrc; backup saved to $backup"
  else
    rm -f "$tmp_file"
  fi
}
...
load_nvm() {
  ...
  if has_cmd nvm; then
    nvm use --delete-prefix default --silent >/dev/null 2>&1 || nvm use --delete-prefix node --silent >/dev/null 2>&1 || true
  fi
}
...
clear_managed_npm_prefix() {
  current_prefix="$(npm config get prefix --location=user 2>/dev/null || true)"
  if [[ "$current_prefix" == "$NPM_PREFIX" ]]; then
    log "removing managed npm prefix from user npm config"
...
write_shell_env_file() {
  ...
  log "writing shell env file: $SHELL_ENV_FILE"
  {
    ...
    printf 'unset NPM_CONFIG_PREFIX npm_config_prefix\n'
