...
3. 检查依赖
   ...
   - Codex auth：没有 `OPENAI_API_KEY` 且没有 `$CODEX_HOME/auth.json` 时交互选择 device auth / browser auth / skip
   - 非交互环境默认用 device auth；也可用 `CODEX_LOGIN_MODE=device|browser|skip` 覆盖
...
5. 配置文件
   ...
   - user 授权默认执行 `lark-cli auth login --domain contact,im,docs,drive,base,sheets,wiki`；可用 `LARK_AUTH_DOMAINS` 覆盖
   - `owner_open_id` 默认通过 `lark-cli auth status` 或 `lark-cli contact +get-user --as user` 自动获取；如果 user identity 缺失，会提示执行 user 授权后重试
   ...
   - App Secret 写入前会调用飞书 `app_access_token` 接口验证；脚本会依次尝试 `open.feishu.cn`、`open.larkoffice.com`、`open.larksuite.com`
   - 已有配置也会在配置阶段校验；验证失败时会提示重新输入并覆盖错误值
   - 如果确认后台凭证无误，也可以选择跳过部署期校验继续写入；后续 `--probe-lark` 仍会验证真实消息权限
